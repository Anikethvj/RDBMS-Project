...
class customer:
    def __init__(self, root):
        ...
        DFrameL=LabelFrame(DFrame, bd=2, width=1000, height=600, padx=20, bg="#1e3d59", relief=RIDGE, font=('Arial', 20, 'bold'), text="Customer Information\n", fg="#f5f0e1")
        DFrameL.pack(side=LEFT)

        DFrameR=LabelFrame(DFrame, bd=2, width=450, height=300, padx=31, pady=3, bg="#1e3d59", relief=RIDGE, font=('Arial', 20, 'bold'), text="Customer Details\n", fg="#f5f0e1")
        DFrameR.pack(side=RIGHT)

        #Labels & Entry Box

        self.lblcustomer_id=Label(DFrameL, font=('Arial', 18, 'bold'), text="Customer ID:", padx=2, pady=2, bg="#1e3d59", fg="#f5f0e1")
        self.lblcustomer_id.grid(row=0, column=0, sticky=W)
        self.txtcustomer_id=Entry(DFrameL, font=('Arial', 18, 'bold'), textvariable=customer_id, width=39, bg="#1e3d59", fg="#f5f0e1")
        self.txtcustomer_id.grid(row=0, column=1) 

        self.lblname=Label(DFrameL, font=('Arial', 18, 'bold'), text="Name:", padx=2, pady=2, bg="#1e3d59", fg="#f5f0e1")
        self.lblname.grid(row=1, column=0, sticky=W) 
        self.txtname=Entry(DFrameL, font=('Arial', 18, 'bold'), textvariable=name, width=39, bg="#1e3d59", fg="#f5f0e1")
        self.txtname.grid(row=1, column=1)

        self.lblgender=Label(DFrameL, font=('Arial', 18, 'bold'), text="Gender:", padx=2, pady=2, bg="#1e3d59", fg="#f5f0e1")
        self.lblgender.grid(row=2, column=0, sticky=W) 
        self.txtgender=Entry(DFrameL, font=('Arial', 18, 'bold'), textvariable=gender, width=39, bg="#1e3d59", fg="#f5f0e1")
        self.txtgender.grid(row=2, column=1)

        self.lblgender=Label(DFrameL, font=('Arial', 18, 'bold'), text="Phone Number:", padx=2, pady=2, bg="#1e3d59", fg="#f5f0e1")
        self.lblgender.grid(row=3, column=0, sticky=W) 
        self.txtgender=Entry(DFrameL, font=('Arial', 18, 'bold'), textvariable=ph_no, width=39, bg="#1e3d59", fg="#f5f0e1")
        self.txtgender.grid(row=3, column=1)

        self.lblemail=Label(DFrameL, font=('Arial', 18, 'bold'), text="E-mail:", padx=2, pady=2, bg="#1e3d59", fg="#f5f0e1")
        self.lblemail.grid(row=4, column=0, sticky=W) 
        self.txtemail=Entry(DFrameL, font=('Arial', 18, 'bold'), textvariable=email, width=39, bg="#1e3d59", fg="#f5f0e1")
        self.txtemail.grid(row=4, column=1)

        self.lblpassword=Label(DFrameL, font=('Arial', 18, 'bold'), text="Password:", padx=2, pady=2, bg="#1e3d59", fg="#f5f0e1")
        self.lblpassword.grid(row=5, column=0, sticky=W) 
        self.txtpassword=Entry(DFrameL, font=('Arial', 18, 'bold'), textvariable=password, width=39, bg="#1e3d59", fg="#f5f0e1")
        self.txtpassword.grid(row=5, column=1)

        self.lblbookdatetime=Label(DFrameL, font=('Arial', 18, 'bold'), text="Booking DateTime:", padx=2, pady=2, bg="#1e3d59", fg="#f5f0e1")
        self.lblbookdatetime.grid(row=6, column=0, sticky=W) 
        self.txtbookdatetime=Entry(DFrameL, font=('Arial', 18, 'bold'), textvariable=bookdatetime, width=39, bg="#1e3d59", fg="#f5f0e1")
        ...
        customerList=Listbox(DFrameR, width=41, height=16, font=('Arial', 12, 'bold'), bg="#1e3d59", fg="#f5f0e1", yscrollcommand=sb.set)
        customerList.bind('<<ListboxSelect>>', custmanager)
        customerList.grid(row=0, column=0, padx=8)
        sb.config(command=customerList.yview)

        #Buttons
        self.btnadd=Button(BFrame, text="Add New", font=('Arial', 20, 'bold'), width=10, height=1, bd=4, bg="#f5f0e1", command=adddata)
        ...
        self.btnclc=Button(BFrame, text="Clear", font=('Arial', 20, 'bold'), width=10, height=1, bd=4, bg="#f5f0e1", command=clcdata)
        self.btnclc.grid(row=0, column=2)

        self.btndel=Button(BFrame, text="Delete", font=('Arial', 20, 'bold'), width=10, height=1, bd=4, bg="#f5f0e1", command=deldata)
        self.btndel.grid(row=0, column=4)

        self.btnup=Button(BFrame, text="Update", font=('Arial', 20, 'bold'), width=10, height=1, bd=4, bg="#f5f0e1", command=updata)
